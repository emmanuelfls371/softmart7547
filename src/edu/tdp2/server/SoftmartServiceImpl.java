...
public class SoftmartServiceImpl extends RemoteServiceServlet implements SoftmartService
{
	...
	public String calificar(CalificacionDto dto)
	{
		...
		try
		{
			...
			if (c != null)
			{
				...
				if (calif != null)
				{
					if (c.getProyecto().getUsuario().getLogin().equals(dto.getUsuario()))
					{ // Usuario es el que publico
						c.setCalifAlVendedor(calif); // El comprador califica al vendedor
						updateNivel(sess, calif, c.getOfertaGanadora().getUsuario());
					}
					else if (c.getOfertaGanadora().getUsuario().getLogin().equals(dto.getUsuario()))
					{ // El que oferto
	...
	public MyAccountDto getMyAccountData(String login)
	{
		...
		try
		{
			...
			comprador
					.setProyectosSinCalificar((List<Proyecto>) sess
							.createQuery(
			...
			comprador
					.setProyectosCancelados((List<Proyecto>) sess
							.createQuery(
									"SELECT DISTINCT proy FROM Proyecto AS proy WHERE usuario = ? AND (cancelado = true OR canceladoXAdmin = true)")
							.setParameter(0, usuario).list());
			...
			vendedor.setProyectosCancelados((List<Proyecto>) sess.createQuery(
					"SELECT DISTINCT proy FROM Proyecto proy JOIN FETCH proy.ofertas AS oferta "
							+ "WHERE oferta.usuario =? AND cancelado = true").setParameter(0, usuario).list());
	...
	public Oferta getOfertaGanadora(Long id)
	{
		...
		try
		{
			Oferta offer = (Oferta) sess
					.createQuery("SELECT ofertaGanadora FROM Contrato AS c WHERE c.proyecto.id = ?")
	...
	public List<Proyecto> getProyectosDestacados()
	{
		...
		try
		{
			List<Proyecto> lista = (List<Proyecto>) sess
					.createQuery(
							"FROM Proyecto WHERE destacado = true AND revisado = true AND cancelado = false AND canceladoXAdmin = false")
					.list();
			if (lista != null)
			{
	...
	public String getUsuario(String login)
	{
		...
		try
		{
			Usuario user = (Usuario) sess.createQuery("FROM Usuario WHERE login = ?").setString(0, login)
					.uniqueResult();
